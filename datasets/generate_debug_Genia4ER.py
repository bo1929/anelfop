...
train = betterConllReader(raw_data_dir, "Genia4ER.train", ["words", "ne"]).iob_sents()
test = betterConllReader(raw_data_dir, "Genia4ER.test", ["words", "ne"]).iob_sents()
...
with open(
    tokenized_data_dir + "debug_Genia4ER_train.tokenized", "w"
) as fp:  # serializing
    json.dump(tokenized[0][0][:500], fp)
with open(tokenized_data_dir + "debug_Genia4ER_train.tags", "w") as fp:  # serializing
    json.dump(tokenized[0][1][:500], fp)

with open(
    tokenized_data_dir + "debug_Genia4ER_test.tokenized", "w"
) as fp:  # serializing
    json.dump(tokenized[1][0][:500], fp)
with open(tokenized_data_dir + "debug_Genia4ER_test.tags", "w") as fp:  # serializing
    json.dump(tokenized[1][1][:500], fp)
