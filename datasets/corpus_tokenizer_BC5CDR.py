...
train = betterConllReader(raw_data_dir, "BC5CDR_train.tsv", ["words", "ne"]).iob_sents()
test = betterConllReader(raw_data_dir, "BC5CDR_test.tsv", ["words", "ne"]).iob_sents()
...
with open(tokenized_data_dir + "BC5CDR_train.tokenized", "w") as fp:  # serializing
    json.dump(tokenized[0][0], fp)
with open(tokenized_data_dir + "BC5CDR_train.tags", "w") as fp:  # serializing
    json.dump(tokenized[0][1], fp)

with open(tokenized_data_dir + "BC5CDR_test.tokenized", "w") as fp:  # serializing
    json.dump(tokenized[1][0], fp)
with open(tokenized_data_dir + "BC5CDR_test.tags", "w") as fp:  # serializing
