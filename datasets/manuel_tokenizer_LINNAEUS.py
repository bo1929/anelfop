...
train = betterConllReader(raw_data_dir, "LINNAEUS.train", ["words", "ne"]).iob_sents()
test = betterConllReader(raw_data_dir, "LINNAEUS.test", ["words", "ne"]).iob_sents()
...
with open(tokenized_data_dir + "LINNAEUS_train.tokenized", "w") as fp:  # serializing
    json.dump(tokenized[0][0], fp)
with open(tokenized_data_dir + "LINNAEUS_train.tags", "w") as fp:  # serializing
    json.dump(tokenized[0][1], fp)

with open(tokenized_data_dir + "LINNAEUS_test.tokenized", "w") as fp:  # serializing
    json.dump(tokenized[1][0], fp)
with open(tokenized_data_dir + "LINNAEUS_test.tags", "w") as fp:  # serializing
