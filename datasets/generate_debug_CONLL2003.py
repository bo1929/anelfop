...
with open(
    tokenized_data_dir + "debug_CONLL2003_train.tokenized", "w"
) as fp:  # serializing
    json.dump(tokenized[0][0][:50], fp)
with open(tokenized_data_dir + "debug_CONLL2003_train.pos", "w") as fp:
    json.dump(tokenized[0][1][:50], fp)
with open(tokenized_data_dir + "debug_CONLL2003_train.tags", "w") as fp:
    json.dump(tokenized[0][2][:50], fp)

with open(tokenized_data_dir + "debug_CONLL2003_test.tokenized", "w") as fp:
    json.dump(tokenized[1][0][:50], fp)
with open(tokenized_data_dir + "debug_CONLL2003_test.pos", "w") as fp:
    json.dump(tokenized[1][1][:50], fp)
with open(tokenized_data_dir + "debug_CONLL2003_test.tags", "w") as fp:
    json.dump(tokenized[1][2][:50], fp)


with open(tokenized_data_dir + "debug_CONLL2003_dev.tokenized", "w") as fp:
    json.dump(tokenized[2][0][:50], fp)
with open(tokenized_data_dir + "debug_CONLL2003_dev.pos", "w") as fp:
    json.dump(tokenized[2][1][:50], fp)
with open(tokenized_data_dir + "debug_CONLL2003_dev.tags", "w") as fp:
    json.dump(tokenized[2][2][:50], fp)
